...
# Date: June 2, 2021
#
# Script Name: GMMAT-Permutation-Reproducible.R
#
...
#permutation: we're going to randomly assign a "treatment" to each sample 
#we can use sample, and job id to set seed

set.seed(jobid)
# treatment = sample(c(1:672),672, replace = F)#this shuffled the phenotypes- here we'll be shuffling the gene ids
###shuffle rows
pheno$fitid <- pheno$fitid[sample(nrow(pheno))]
...
#run GMMAT
outfile = paste(outpath,"perm",jobid,sep = "")
